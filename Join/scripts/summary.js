import { firebaseConfig } from "./firebase-config.js"; // Adjust the path accordingly
...
function handleLoggedOutUser(
  ...
  if (document.getElementById("welcomeText-mobile"))
...
async function loadTaskCounts(userId) {
  try {
    const { taskCounts, totalTasks, upcomingDeadline } =
      await initializeTaskCounts(userId);
...
async function fetchTaskCountsFromFolders(userId, folders) {
  ...
  for (let [key, folderName] of Object.entries(folders)) {
    ...
    if (folderSnapshot.exists()) {
      const { tasks, urgent, deadline } = countUrgentTasks(
        folderSnapshot.val()
      );
...
async function initializeTaskCounts(userId) {
  ...
  const { taskCounts, totalTasks, upcomingDeadline } =
    await fetchTaskCountsFromFolders(userId, folders);
  if (taskCounts.urgent > 1 && !upcomingDeadline) {
    upcomingDeadline =
      urgentTasks[Math.floor(Math.random() * urgentTasks.length)].duedate;
  }
  return { taskCounts, totalTasks, upcomingDeadline };
}
...
function countUrgentTasks(tasks) {
  ...
  for (let task of Object.values(tasks)) {
    if (task.prio === "Urgent") {
      ...
      if (
        !upcomingDeadline ||
        new Date(task.duedate) < new Date(upcomingDeadline)
      ) {
...
function updateTaskCountsDisplay(taskCounts, totalTasks, upcomingDeadline) {
  document.getElementById("todo-task-count").textContent =
    taskCounts.todos || 0;
  document.getElementById("done-task-count").textContent =
    taskCounts.donetasks || 0;
  document.getElementById("total-task-count").textContent = totalTasks || 0;
  document.getElementById("inprogress-task-count").textContent =
    taskCounts.inprogress || 0;
  document.getElementById("review-task-count").textContent =
    taskCounts.awaitingfeedback || 0;
  document.getElementById("urgent-task-count").textContent =
    taskCounts.urgent || 0;
  document.getElementById("due-date").textContent =
    upcomingDeadline || "No Date";
}
