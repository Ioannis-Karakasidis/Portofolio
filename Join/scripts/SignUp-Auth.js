...
function handleSignUp(event) {
  ...
  createUserWithEmailAndPassword(auth, email, password)
    .then((userCredential) =>
      createUserDocument(userCredential.user, email, name)
    )
...
function handleSignUpError(error) {
  ...
  const message =
    errorMessages[error.code] || "Unable to create User: " + error.message;
...
function validateSignUpForm() {
  ...
  const validPassword =
    password && confirmPassword && validatePassword(password, confirmPassword);
...
document
  .querySelectorAll(
    "#rEmail, #rPassword, #rConfirmPassword, #rName, #checkPrivacy"
  )
